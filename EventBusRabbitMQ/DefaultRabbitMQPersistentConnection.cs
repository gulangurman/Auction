    ...
    public class DefaultRabbitMQPersistentConnection : IRabbitMQPersistentConnection
    {
        ...
        public bool TryConnect()
        {
            ...
            var policy = RetryPolicy.Handle<SocketException>()
                ...
                {
                    _logger.LogWarning(ex, "RabbitMQ Client could not connect in {Timeout}s", $"{time.TotalSeconds:n1}");
            ...
            {
                _logger.LogCritical("FATAL ERROR: failed to create RabbitMQ connection");
                return false;
            }
        }
        ...
        {
            if (_disposed) return;
            _logger.LogWarning("A RabbitMQ connection has thrown an exception. Trying to reconnect...");
            TryConnect();
        }
        ...
        {
            if (_disposed) return;
            _logger.LogWarning("A RabbitMQ connection is blocked. Trying to reconnect...");
            TryConnect();
        }
